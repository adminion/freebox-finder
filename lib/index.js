...
var banner = fs.readFileSync('lib/banner'),
    license = fs.readFileSync('lib/LICENSE')
...
function FreeboxFinder_Server (config, env) {
    ...
    app.get('/', function (request, response) {
        response.render('index', { license: license });
    });
    ...
    function ioConnection (socket) {
        ...
        function socketGetBoxes (cityState, tags) {
            ...
            debug('gettings boxes in ' + cityState);

            // if the socket has a 'cityState' code, and it has changed
            if ('cityState' in socket && socket.cityState !== cityState) {
                // let's remove it from that room, so it doesn't get bombarded
                // with new box notifications for old cityState codes
            ...
            var city = cityState.shift;
            var state = cityState;

            // build the boxes query
            var query = Box.model
                .where('location.address_components.locality.long_name', city)
                .where('location.address_components.locality.administrative_area_level_1.short_name', state);
        ...
        function socketNewBox(newBox) {
            ...
            box.save(function newBoxSaved(err, savedBox) {
                ...
                // if the box was saved successfully
                    ...
                    self.io.in(newBox.location.cityState()).emit('new-box', savedBox.toObject());
                }
            });       
        }
